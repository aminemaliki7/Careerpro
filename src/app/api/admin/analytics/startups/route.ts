...
import { getStartupsAnalytics } from '@/lib/analytics/queries';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = await getStartupsAnalytics(period);
    return ok(payload);
  } catch (error) {
    if (error instanceof PeriodValidationError) return badRequest(error.message);
    return serverError(error, 'admin-analytics-startups');
  }
}